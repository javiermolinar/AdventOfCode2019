...
namespace ConsoleApp5
{
    class Program
    {
        ...
        private static int CalculateMinDistance(Dictionary<string, int> map){
         ...
         public static void WritePath(List<string>path, Dictionary<string,int> dictionary, bool intersec = false)
         {    
            int posI = 0;
            int posJ = 0;

            foreach(var wire in path)
            {
                char direction = wire[0];
                int positions = int.Parse(wire.Substring(1));
                
                if ( direction == 'U'){
                   for(int i=0;i<positions;i++){
                      posI--;
                      SetMap(dictionary,posI,posJ,intersec);                    
                   }                   
                }
                if ( direction == 'R'){
                   for(int i=0;i<positions;i++){
        ...
        private static void SetMap(Dictionary<string,int> dictionary,int i, int j, bool intersec = false)
        {
            if (!dictionary.ContainsKey(GetKey(i, j)))
            {
                 dictionary.Add(GetKey(i,j),1);                
            }
            else if(intersec)
            {
               dictionary[GetKey(i,j)]++;
            }
        }      
    }
}
